...
def adjust_predict(all_means, base_result,hometype):
...
def predict_price(zipcode,test_date,hometype,all_parameters,all_means):
     ...
     if (type(base_result)!=str):
          final_result = adjust_predict(all_means, base_result,hometype)
          return(final_result)
     else:
          return(base_result)

# testcase
all_parameters = pd.read_csv('base_parameters.csv',converters={'zipcode': str})
all_means = pd.read_csv('adjust_means.csv',converters={'zipcode': str})
zipcode = '10025'
test_date = '2018-10'
hometype='bed2'
predict_price(zipcode,test_date,hometype,all_parameters,all_means)
